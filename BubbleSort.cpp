...
void nhapNhayHaiHinhChuNhat(int trai1, int tren1, int phai1, int duoi1, int giatri1,int trai2, int tren2, int phai2, int duoi2, int giatri2) {
...
void sapXepNoiBot(int a[], int n, int luachon) {
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - 1 - i; j++) {
            ...
            if ((luachon == 1 && a[j] > a[j + 1]) || (luachon == 2 && a[j] < a[j + 1])) {
                nhapNhayHaiHinhChuNhat(trai_j, tren, phai_j, duoi, a[j], trai_j1, tren, phai_j1, duoi, a[j + 1]);
    ...
    // Làm nổi bật phần tử đầu tiên sau khi sắp xếp xong
    setfillstyle(SOLID_FILL, GREEN);
