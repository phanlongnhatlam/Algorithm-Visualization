...
void themPhanTu(Node* table[], int key) {
    ...
    char msg[50];
    sprintf_s(msg,"Key: %d, Hash: %d", key, index);
...
void xoaPhanTu(Node* table[], int key) {
    ...
    while (temp != nullptr) {
        if (temp->data == key) {
            // nếu là phần tử đầu tiên thì trỏ tới phần tử tiếp theo nó
            if (prev == nullptr) {
...
int nhapSo() {
    int key;
    cout << "Xin moi ban nhap gia tri : ";
    cin >> key;
    return key;
}
    int main() {
    initwindow(1800, 1200, "Bang Bam");
